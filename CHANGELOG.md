# Changelog

Все значимые изменения в этом проекте будут документированы в этом файле.

Формат основан на [Keep a Changelog](https://keepachangelog.com/ru/1.0.0/),
и этот проект придерживается [Semantic Versioning](https://semver.org/lang/ru/).

## [1.5.0] - 2026-05-30

### Добавлено

- Добавлен эндпоинт `GET /recommendations`
- Добавлена интеграция с Neo4j
- Добавлено кэширование рекомендаций в Redis
- Добавлен pre-commit
- Добавлено окружение для smoke тестов
- Добавлены команды для линтера в makefile

### Изменено

- Обновлена коллекция smoke-тестов
- Обновлена документация проекта

## [1.4.0] - 2026-05-13

### Добавлено

- Добавлены эндпоинты для отзывов на мероприятия:
  - `POST /events/{event_id}/reviews` — создание отзыва
  - `GET /events/{event_id}/reviews` — список отзывов с пагинацией
  - `PATCH /events/{event_id}/reviews/{review_id}` — редактирование отзыва владельцем
- Добавлен параметр `?include=reviews` на эндпоинты:
  - `GET /events`,
  - `GET /events/{event_id}`,
  - `GET /users/{user_id}/events`
- Добавлена таблица `event_reviews` в Cassandra
- Добавлено кэширование агрегатов по отзывам в Redis

## [1.3.0] - 2026-04-25

### Добавлено

- Добавлены эндпоинты `POST /events/{event_id}/like` и `POST /events/{event_id}/dislike` для реакций на мероприятия
- Добавлен параметр `?include=reactions` на эндпоинты:
  - `GET /events`,
  - `GET /events/{event_id}`
  - `GET /users/{user_id}/events`
- Добавлена интеграция с Apache Cassandra
- Добавлено кэширование счётчиков реакций в Redis


## [1.2.0] - 2026-04-10

### Добавлено

- Развёрнут шардированный кластер MongoDB в `docker-compose.yml`
- Добавлен скрипт для инициализации реплик и шардирования коллекций
- Добавлены эндпоинты `GET /users`, `GET /users/{user_id}` и `GET /users/{user_id}/events`
- Добавлена фильтрация и валидация событий по категории, цене и городу

## [1.1.0] - 2026-03-14

### Добавлено
- Добавлен эндпоинт `/session` для создания и обновления анонимной сессии
- Добавлено хранение сессий в Redis
- Добавлена Postman-коллекция для проверки API
- Добавлена конфигурация Redis в `docker-compose.yml`

### Изменено

- Обновлен `GET /health`. Эндпоинт возвращает cookie, но не создает и не обновляет сессию


## [1.0.1] - 2026-02-14

### Добавлено

- Инициализирован FastAPI сервис со структурой проекта через src-layout
- Реализован application factory (`create_app`)
- Добавлена модульная система роутеров
- Реализован health-check endpoint
- Добавлено middleware с приветственным сообщением на корневом пути
- Подключена конфигурация через `.env.local`
- Добавлена поддержка python-dotenv для локального запуска
- Реализована загрузка порта из переменных окружения
- Настроена сборка и запуск через Docker и docker-compose
- Добавлен запуск сервиса через Makefile (`make run`)
- Исправлена проблема с env-переменными в docker compose
- Настроен mypy для src-структуры проекта
- Подготовлен контейнерный запуск через uvicorn


## [1.0.0] - 2026-02-06

Первая версия шаблона проекта
для выполнения лабораторных работ по курсу NoSQL баз данных (см. [ndbx](https://github.com/sitnikovik/ndbx))

### Добавлено

- *.env.local*: файл для хранения конфигурации проекта
- *.gitignore*: файл для указания игнорируемых файлов и папок (для популярных языков программирования)
- *.labrc*: файл для указания текущей лабораторной работы
- *CHANGELOG.md*: файл для ведения истории изменений проекта
- *CODEOWNERS*: файл для указания ответственных за различные части проекта
- *CONTRIBUTING.md*: руководство для разработчиков по внесению изменений в проект
- *Makefile*: файл для автоматизации запуска и остановки проекта
- *README.md*: файл с описанием проекта, его целей и инструкциями по использованию
- *.github/workflows/: воркфлоу для автоматической проверки лабораторных работ в GitHub Actions
- *eventhub.yml*: проверка лабораторных работ по проекту EventHub
- *.github/scripts/*: папка для хранения скриптов, используемых в воркфлоу GitHub Actions
- *lab_number.sh*: скрипт для получения номера текущей лабораторной работы из файла *.labrc*
